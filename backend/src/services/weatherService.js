...
async function getCoordinates(location) {
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      console.error("No coordinates found for:", location, data);
      return null;
    }
...
export async function getWeatherAdvisory(location) {
  try {
    ...
    if (!coords) {
      return `❌ Sorry, I couldn’t find coordinates for "${location}". Try another nearby city.`;
    }

    const { lat, lon } = coords;
    const weatherUrl = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&appid=${WEATHER_API_KEY}&units=metric`;
